  ...
  return (
    <div className="chat-theme-input relative inset-auto z-[90] border-t-2 border-[#d7cdbd] bg-[#fffdf8] text-zinc-950 px-2.5 py-2 pb-[calc(env(safe-area-inset-bottom)+0.55rem)] shadow-[0_-10px_30px_rgba(0,0,0,0.14)] md:z-[80] md:border-t md:border-zinc-200 md:bg-white md:px-4 md:py-3">
      ...
      {(showEmojiPicker || showAnimationPicker) && (
        ...
        >
          <div className="grid grid-cols-2 gap-1.5 border-b border-zinc-200 bg-zinc-50 p-1.5">
            <button type="button" onClick={() => { setMobileMediaPickerTab("emoji"); setShowEmojiPicker(true); setShowAnimationPicker(false); }} className={`flex items-center justify-center gap-2 rounded-xl px-2 py-2.5 text-[10px] font-black ${mobileMediaPickerTab === "emoji" ? "bg-zinc-900 text-white" : "text-zinc-600 hover:bg-zinc-100"}`}><Smile className="h-4 w-4" />EMOJİLER</button>
            <button type="button" onClick={() => { setMobileMediaPickerTab("animation"); setShowAnimationPicker(true); setShowEmojiPicker(false); }} className={`flex items-center justify-center gap-2 rounded-xl px-2 py-2.5 text-[10px] font-black ${mobileMediaPickerTab === "animation" ? "bg-violet-600 text-white" : "text-zinc-600 hover:bg-zinc-100"}`}><Sparkles className="h-4 w-4" />ANİMASYONLAR</button>
          </div>
